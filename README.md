...
*(Drop a screenshot of your Reward/Episode Length training graph here)*
![Training Graph]()

### 3. Domain Randomization & "Death Spawns"

To ensure the AI was robust, the starting positions were randomized (X-offset, altitude, angle, and starting drop speed). However, early iterations spawned the rocket in mathematically impossible scenarios (e.g., 30 meters off-center, upside down, falling at 100 m/s).
The randomization was carefully calibrated into a discrete array grid to ensure every drop was physically solvable, allowing the moving average of the reward function to finally stabilize. The final model was trained for over **15.8 Million steps**.
