...
class Phase3App(gym.Env):
    ...
    def _get_state(self):
        ...
        ideal_vy = -(math.sqrt(max(0.0, true_altitude)) * 4.0) - 1.5
    ...
    def _compute_reward(self, state):
        ...
        # Step 1: A gentler curve. (Changed 4.0 to 2.0, and -2.0 to -1.0)
        # At 2 meters, this now asks for a completely safe -3.8 m/s!
        ideal_vy = -(math.sqrt(max(0.0, true_altitude)) * 4.0) - 1.5
        
        # Step 2: Calculate the exact absolute difference
        speed_error = abs(ideal_vy - vel.y)
        
        # Step 3: If the rocket is within 1.0 m/s of the perfect curve, give a cookie!
        if speed_error < 1.0:
            reward += 0.05
            
        # ---> THE FIX: The Two-Sided Penalty <---
        # Step 4: Penalize for deviating (Too fast OR too slow!)
